...
export const getAssignedCollaboratorDataController = myControllerHandler(
  async (req, res) => {
    ...
    console.log('Incoming request to getAssignedCollaboratorDataController');
    ...
    for (let i = 0; i < dataOfCollaborators.length; i++) {
      const user = dataOfCollaborators[i];
      const collabRecord = collaborationDataOfUser.find(
        collab => collab.collaboratorId === user.id
      );

      refinedDataOfCollaborator.push({
        id: user.id,
        email: user.email,
        fullName: user.fullName,
        phoneNumber: user.phoneNumber,
        profileImageUrl: user.profileImageUrl,
        dateOfAdding: collabRecord?.createdAt || null, // Assuming createdAt stores the addition date
      });
    }
